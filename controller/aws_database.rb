...
module RioDbLib
	...
	def self.authenticate_aws_user u_name, password
		sql_client 	= get_client
		sql_query 	= "SELECT EXISTS (SELECT * FROM sketchup_users WHERE email='#{u_name}' AND pwd = '#{password}');"
		get_name = "select user_name from sketchup_users where email='#{u_name}';"
		...
		if result
			get_aws_keys sql_client, nameresp.entries
			return true
		end
		return false
	end
